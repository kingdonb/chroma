...
// SerialisableMutator is a Mutator that can be serialised and deserialised to/from JSON.
...
type multiMutator struct {
	Mutators []Mutator `json:"mutators" xml:"mutator"`
}
...
func (m *multiMutator) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	for _, m := range m.Mutators {
		sm, ok := m.(SerialisableMutator)
		if !ok {
			return fmt.Errorf("multi-mutator element is not serialisable")
		}
		if err := e.EncodeElement(sm, xml.StartElement{Name: xml.Name{Local: sm.MutatorKind()}}); err != nil {
			return err
		}
	}
...
type combinedMutator struct {
	States []string `json:"states" xml:"state,attr"`
}
...
type popMutator struct {
	Depth int `json:"depth" xml:"depth,attr"`
}
